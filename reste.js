var main = function() {
    ...
    function makeHttpRequest(args, onLoad, onError) {
        ...
        if (args.url.indexOf("http") >= 0) {
            ...
            http.open(
                args.method,
                config.url ? config.url + args.url : args.url
            );
        }
        ...
        if (args.headers) {
            ...
            for (var header in args.headers) {
                if (
                    header === "Content-Type" &&
                    args.headers[header] === "application/x-www-form-urlencoded"
                ) {
                    formEncode = true;
                } else if (
                    header === "Content-Type" &&
                    args.headers[header] === "application/json"
                ) {
                    formEncode = false;
                }
                ...
                http.setRequestHeader(
                    header,
                    typeof args.headers[header] === "function"
                        ? args.headers[header]()
                        : args.headers[header]
                );

                log(
                    "Setting local header - " +
                        header +
                        ": " +
                        (typeof args.headers[header] === "function"
                            ? args.headers[header]()
        ...
        http.onerror = function(e) {
            ...
            if (config.errorsAsObjects) {
                ...
                warn(
                    "Future versions of RESTe will return errors as objects. Use config.errorsAsObjects = true to support this now and update your apps!"
                );
            }
    ...
    reste.addMethod = function(args) {
        ...
        reste[args.name] = function(params, onLoad, onError) {
            ...
            if (args.expects) {
                ...
                args.expects.forEach(function(expectedParam) {
                    if (
                        method === "POST" && params.body
                            ? !params.body[expectedParam]
                            : !params[expectedParam]
                    ) {
                        throw "RESTe :: missing parameter " +
                            expectedParam +
                            " for method " +
                            args.name;
                    }
                ...
                if (missing.length > 0) {
                    throw "RESTe :: missing parameter/s " +
                        missing +
                        " for method " +
                        args.name;
                } else {
    ...
    reste.createModel = function(name, attributes) {
        ...
        if (
            reste.modelConfig &&
            reste.modelConfig[name] &&
            reste.modelConfig[name].transform
        ) {
        ...
        reste.addModel = function(args) {
            ...
            if (args.collections) {
                args.collections.forEach(function(collection) {
                    Alloy.Collections[collection.name] =
                        Alloy.Collections[collection.name] ||
                    ...
                    function(response) {
                        if (response != null && response != undefined) {
                            ...
                            if (response[collectionConfig.content]) {
                                response[collectionConfig.content].forEach(
                                    function(item) {
                                ...
                                if (options.success)
                                    options.success(
                                        response[collectionConfig.content]
                                    );
